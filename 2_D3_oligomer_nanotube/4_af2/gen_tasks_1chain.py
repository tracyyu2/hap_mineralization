...
for fa in glob(args.input_directory + '/output/alignments/*fa'):
    with open(fa, 'r') as fin:
        lline = fin.readline()  # Skip first line (FASTA header)
        lline = fin.readline()  # Get second line with sequence information
        ...
        for i in range(nseqs):
            ...
            lline = fin.readline()  # Skip line with sequence info
            lline = fin.readline()  # Sequence line (we will take the first chain only)
            pname = fa.split('/')[-1][:-3]  # Filename without extension

            fout_path = os.path.join(args.output_directory, 'fastas%d' % out_ind, f"{pname}_{i}.fa")
            with open(fout_path, 'w') as fout:
                fout.write(f">{pname}_{i}\n")

                # Take only the first sequence (first chain)
                # Split the sequence by '/' and only take the first part
                sequence = lline.split('/')[0]
                
                fout.write(sequence)  # Write only the first sequence

            fout_tasks.write("/software/containers/colabfold.sif colabfold_batch " +
                             '--msa-mode single_sequence --pair-mode unpaired_paired ' +
                             '--model-type alphafold2_multimer_v3 --num-recycle 3 ' +
